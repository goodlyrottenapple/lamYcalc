...
(x ∷ τᵢ) ≟TIₗ (.x ∷ τⱼ) | yes refl | no τᵢ≠τⱼ =
  no (λ ∩x∷τᵢ≡∩x∷τⱼ → τᵢ≠τⱼ (proj₂ (∷-injective ∩x∷τᵢ≡∩x∷τⱼ)))
(x ∷ τᵢ) ≟TIₗ (y ∷ τⱼ) | no x≠y | _ =
  no (λ ∩x∷τᵢ≡∩y∷τⱼ → x≠y (proj₁ (∷-injective ∩x∷τᵢ≡∩y∷τⱼ)))
------------------------------------------------------------------------------------
...
⊆ₗ-⊆ {τᵢ = τ ∷ τᵢ} τᵢ⊆τⱼ τⱼ∷A =
  ...
  ⊆-refl (∷'ₗ-∈ (τᵢ⊆τⱼ (here refl)) τⱼ∷A)) (⊆ₗ-⊆ (λ {x} z → τᵢ⊆τⱼ (there z)) τⱼ∷A)
------------------------------------------------------------------------------------
...
⊆-∷'ₗ {τᵢ = x ∷ τᵢ} x∷τᵢ⊆τⱼ τⱼ∷' = cons
  ...
  (⊆-∷'ₗ {τᵢ = τᵢ} (λ {x₁} z → x∷τᵢ⊆τⱼ (there z)) τⱼ∷')
------------------------------------------------------------------------------------
...
⊆ₗ++-l {τᵢ = τ ∷ τᵢ} {τⱼ} {τⱼ'} τⱼ⊆τⱼ' (cons x τᵢ∷'ₗ) =
  cons
    ...
    (⊆ₗ-trans {τⱼ = τᵢ ++ τⱼ'}
      (⊆ₗ++-l τⱼ⊆τⱼ' τᵢ∷'ₗ)
      (⊆ₗ-⊆ (λ {x₁} → there) (++-∷'ₗ (cons x τᵢ∷'ₗ) (⊆ₗ-∷'ₗ-r τⱼ⊆τⱼ'))))
------------------------------------------------------------------------------------
...
mon {τ' = τ'} τ⊆τ' τᵢ⊆τᵢ' = glb
  (⊆ₗ-trans
    τ⊆τ'
    (⊆ₗ-⊆ (λ x₁ → ∈-cons-l _ x₁) (∷'ₗ-++ (⊆ₗ-∷'ₗ-r τ⊆τ') (⊆ₗ-∷'ₗ-r τᵢ⊆τᵢ'))))
  (⊆ₗ-trans
    τᵢ⊆τᵢ'
    (⊆ₗ-⊆ (λ x₁ → ∈-cons-r τ' x₁) (∷'ₗ-++ (⊆ₗ-∷'ₗ-r τ⊆τ') (⊆ₗ-∷'ₗ-r τᵢ⊆τᵢ'))))
------------------------------------------------------------------------------------
...
subₗ Γ⊩ₗm∶τ (⊆ₗ-trans τ'⊆τ τ'⊆τ₁) Γ⊆Γ' =
  subₗ (subₗ Γ⊩ₗm∶τ τ'⊆τ₁ Γ⊆Γ') τ'⊆τ (⊆Γ-⊆ (⊆Γ-wfΓ' Γ⊆Γ') (λ {x} z → z))
------------------------------------------------------------------------------------
