...
(τ₁₁ ⟶ τ₁₂) ≟T (.τ₁₁ ⟶ τ₂₂) | yes refl | no τ₁₂≠τ₂₂ = no (λ eq → τ₁₂≠τ₂₂ (⟶-inj-r eq))
...
var-⊢-≡ {x} {.x} {τ₁} {τ₂} {Γ} (var (cons x∉ wf-Γ) x∈x∷Γ) refl | no τ₂≠τ₁ = ⊥-elim (contr x∈x∷Γ)
  where
  contr : (x , τ₂) ∉ (x , τ₁) ∷ Γ
...
weakening Γ⊆Γ' wf-Γ' (app Γ⊢m∶τ Γ⊢n∶τ₁) = app (weakening Γ⊆Γ' wf-Γ' Γ⊢m∶τ) (weakening Γ⊆Γ' wf-Γ' Γ⊢n∶τ₁)
weakening {Γ} {Γ'} Γ⊆Γ' wf-Γ' (abs {_} {τ₁} {τ₂} L {m} cf) =
  abs (L ++ dom Γ') (λ {x} x∉L++Γ' → weakening {(x , τ₁) ∷ Γ} {(x , τ₁) ∷ Γ'} {m ^' x} {τ₂}
    (cons-⊆ Γ⊆Γ') (cons (∉-cons-r L _ x∉L++Γ') wf-Γ') (cf (∉-cons-l _ _ x∉L++Γ')))
weakening Γ⊆Γ' wf-Γ' (Y _) = Y wf-Γ'


wf-Γ-exchange : ∀ {Γ x y τ₁ τ₂} -> Wf-Ctxt ((x , τ₁) ∷ (y , τ₂) ∷ Γ) -> Wf-Ctxt ((y , τ₂) ∷ (x , τ₁) ∷ Γ)
...
subst-⊢ {Γ} {_} {n} {τ₁} {_} {x} (lam L cf) (abs {_} {τ₁'} {τ₂} L' {m} cf') Γ⊢n∶τ₁ = abs (x ∷ (L ++ L' ++ dom Γ)) body
  ...
  body {x'} x'∉ rewrite
    subst-open-var x' x n m (fv-x≠y _ _ x'∉) (⊢-term Γ⊢n∶τ₁) =
      subst-⊢ {(x' , τ₁') ∷ Γ} {m ^' x'} {n} {τ₁} {τ₂} (cf (∉-∷-elim _ (∉-cons-l _ _ x'∉)))
        (exchange (cf' (∉-cons-l _ _ (∉-cons-r L _ (∉-∷-elim _ x'∉)))))
        (weakening there (cons (∉-cons-r L' _ (∉-cons-r L _ (∉-∷-elim _ x'∉))) (⊢-imp-wfΓ Γ⊢n∶τ₁)) Γ⊢n∶τ₁)
...
^-⊢ : ∀ {Γ L m n τ₁ τ₂}  -> ( cf : ∀ {x} -> (x∉L : x ∉ L) -> ((x , τ₁) ∷ Γ) ⊢ m ^' x ∶ τ₂ ) ->
  Γ ⊢ n ∶ τ₁ -> Γ ⊢ m ^ n ∶ τ₂
...
->β-⊢ (app {Γ} {_} {n} (abs L {m} cf) Γ⊢n∶τ₁) (beta term-lam-x term-cf) = ^-⊢ {m = m} cf Γ⊢n∶τ₁
