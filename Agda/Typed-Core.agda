...
Λ*^-*^~ _ _ (app {m = m} {n} {m'} {n'} t~t' t~t'') =
  app (Λ*^-*^~ m m' t~t') (Λ*^-*^~ n n' t~t'')
Λ*^-*^~ _ _ Y = Y
------------------------------------------------------------------------------------
...
data ΛTerm : ∀ {τ} -> Λ τ -> Set where
  ...
  app : ∀ {A B} {e₁ : Λ (A ⟶ B)} {e₂ : Λ A} -> ΛTerm e₁ -> ΛTerm e₂ ->
    ΛTerm (app e₁ e₂)
  Y : ∀ {t} -> ΛTerm (Y t)
------------------------------------------------------------------------------------
...
Λapp-inj-l : ∀ {A B : Type} {t₁ u₁ : Λ (A ⟶ B)} {t₂ u₂ : Λ A} ->
  (Λ.app t₁ t₂) ≡ (Λ.app u₁ u₂) -> t₁ ≡ u₁
Λapp-inj-l refl = refl
------------------------------------------------------------------------------------

Λapp-inj-r : ∀ {A B : Type} {t₁ u₁ : Λ (A ⟶ B)} {t₂ u₂ : Λ A} ->
  (Λ.app t₁ t₂) ≡ (Λ.app u₁ u₂) -> t₂ ≡ u₂
Λapp-inj-r refl = refl
------------------------------------------------------------------------------------

Λapp-eq : ∀ {A B : Type} {t₁ u₁ : Λ (A ⟶ B)} {t₂ u₂ : Λ A} -> t₁ ≡ u₁ -> t₂ ≡ u₂ ->
  (Λ.app t₁ t₂) ≡ (Λ.app u₁ u₂)
Λapp-eq refl refl = refl
------------------------------------------------------------------------------------

^-ΛTerm-eq-aux : ∀ {τ τ' τ''} j (t : Λ τ') i (u : Λ τ'') (e : Λ τ) → ¬ (i ≡ j) ->
  Λ[ j >> t ] e ≡ Λ[ i >> u ] (Λ[ j >> t ] e) ->
  e ≡ Λ[ i >> u ] e
...
Λsubst-open x t k u (fv .x) trm-t | yes refl | yes refl =
  ^-ΛTerm-eq k (u Λ[ x ::= t ]) trm-t
...
Λsubst-open-var2 : ∀ {τ τ' τ''} x y (u : Λ τ') (e : Λ τ) -> ¬ (x ≡ y) -> ΛTerm u ->
  (Λ[ 0 >> fv {τ''} x ] e) Λ[ y ::= u ] ≡ (Λ[ 0 >> fv {τ''} x ] (e Λ[ y ::= u ]))
Λsubst-open-var2 {τ'' = τ''} x y u e x≠y lu rewrite
  Λsubst-open-var {τ'' = τ''} x y u e x≠y lu = refl
------------------------------------------------------------------------------------

Λsubst-fresh : ∀ {τ τ'} x (t : Λ τ') (u : Λ τ) -> (x∉FVt : x ∉ (ΛFV t)) ->
  (t Λ[ x ::= u ]) ≡ t
...
Λsubst-intro x t e x∉ lt | eq | yes refl | yes refl
  rewrite Λsubst-fresh x e t x∉ = sym eq
...
Λ^-^-swap : ∀ {τ τ' τ''} k n x y (m : Λ τ) -> ¬(k ≡ n) -> ¬(x ≡ y) ->
  Λ[ k >> fv {τ'} x ] (Λ[ n >> fv {τ''} y ] m) ≡ Λ[ n >> fv {τ''} y ] (Λ[ k >> fv {τ'} x ] m)
...
Λ^-^-swap k n x y (bv .n) k≠n x≠y | yes refl | yes refl | no _ | yes refl | no τ≠τ =
  ⊥-elim (τ≠τ refl)
...
Λ^-^-swap k n x y (bv .n) k≠n x≠y | yes refl | no τ≠τ'' | no ¬p | yes refl | yes refl =
  ⊥-elim (τ≠τ'' refl)
Λ^-^-swap k n x y (bv .n) k≠n x≠y | yes refl | no τ≠τ'' | no ¬p₁ | yes refl | no _ = refl
Λ^-^-swap k n x y (bv .n) k≠n x≠y | yes refl | no τ≠τ'' | no _ | no n≠n =
  ⊥-elim (n≠n refl)
...
Λ^-^-swap k .k x y (bv .k) k≠n x≠y | no n≠i | no _ | yes refl | no τ≠τ' | yes refl =
  ⊥-elim (k≠n refl)
Λ^-^-swap k n x y (bv .k) k≠n x≠y | no n≠i | no _ | yes refl | no τ≠τ' | no _ = refl
Λ^-^-swap k n x y (bv i) k≠n x≠y | no n≠i | no _ | no k≠i with n ≟ i
Λ^-^-swap k i x y (bv .i) k≠n x≠y | no n≠i | no _ | no k≠i | yes refl =
  ⊥-elim (n≠i refl)
Λ^-^-swap k n x y (bv i) k≠n x≠y | no n≠i | no _ | no k≠i | no _ = refl

Λ^-^-swap k n x y (fv z) k≠n x≠y = refl
Λ^-^-swap {_} {τ'} {τ''} k n x y (lam {B} A m) k≠n x≠y = cong
  (lam A)
  (Λ^-^-swap {B} {τ'} {τ''} (suc k) (suc n) x y m (λ sk≡sn → k≠n (≡-suc sk≡sn)) x≠y)
Λ^-^-swap {_} {τ'} {τ''} k n x y (app {τ₁} {τ₂} t1 t2) k≠n x≠y rewrite
  Λ^-^-swap {τ₁ ⟶ τ₂} {τ'} {τ''} k n x y t1 k≠n x≠y |
  Λ^-^-swap {τ₁} {τ'} {τ''} k n x y t2 k≠n x≠y = refl
Λ^-^-swap k n x y (Y _) k≠n x≠y = refl
------------------------------------------------------------------------------------

ΛFV-^ : ∀ {k x y A B} (m : Λ A) -> x ∉ ΛFV m -> ¬(x ≡ y) ->
  x ∉ ΛFV (Λ[ k >> fv {B} y ] m)
