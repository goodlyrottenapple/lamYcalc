...
wfI-Γ-exchange (cons x∉ x₂ (cons x∉₁ x₃ wf-Γ)) = cons
  ((∉-∷ _ _ (λ y≡x → fv-x≠y _ _ x∉ (≡-sym y≡x)) x∉₁))
  x₃
...
strenghten-⊩-aux {x = x} (un m~Tt) x∉ (abs L {m} cf) =
  abs (x ∷ L)
      (λ x∉L → strenghten-⊩ₗ-aux (^'-~T-inv m~Tt)
                                  (ΛFV-^ m x∉ (λ x₂ → fv-x≠y _ _ x∉L (sym x₂)))
...
aux {A} {B} {Γ} {k = k} (fv x₁) l-fv y∉Γ x∉FVm y∉FVm x≠y x∷Γ⊩m^'x∶τ' =
  sub (strenghten-⊩ (ΛFV-^ {k = k} {A = A} {B} (fv x₁) x∉FVm x≠y) x∷Γ⊩m^'x∶τ')
      (⊆-refl (⊩-∷' x∷Γ⊩m^'x∶τ'))
      (⊆Γ-⊆ (cons y∉Γ (∷'' (⊩-wf-Γ x∷Γ⊩m^'x∶τ')) wf-Γ) (λ {x} → there))
  where
...
aux {A} {_} {Γ} {τ} {τ' ~> τ''} {x} {y} {k} (lam {B} A' m) (un m~Tt) y∉Γ x∉FVm y∉FVm x≠y (abs L cf) =
  ...
  cf' {x'} x'∉L rewrite
    Λ^-^-swap {B} {A} {A'} (suc k) 0 x x' m (λ ()) (λ x₂ -> fv-x≠y _ _ x'∉L (sym x₂)) =
    exchange-⊩ₗ (cf (∉-∷-elim _ x'∉L))
  ...
  body {x'} x'∉L rewrite
    Λ^-^-swap {B} {A'} {A} 0 (suc k) x' y m (λ ()) (λ x₂ -> fv-x≠y _ _ x'∉L x₂) =
    exchange-⊩ₗ (ih' x'∉L)
