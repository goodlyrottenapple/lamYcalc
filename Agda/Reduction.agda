...
data _->||_ : PTerm ↝ PTerm where
  ...
  abs : ∀ L {m m'} -> ( ∀ x -> x ∉ L -> (m ^' x) ->|| (m' ^' x) ) ->
    lam m ->|| lam m'
  beta : ∀ L {m m' n n'} -> ( ∀ {x} -> x ∉ L -> (m ^' x) ->|| (m' ^' x) ) -> n ->|| n' ->
    app (lam m) n ->|| (m' ^ n')
...
->||-Term-l (abs L x) = lam L (λ {y} x∉L → ->||-Term-l (x y x∉L))
->||-Term-l (beta L x m->||m') =
...
lem2-5-1 _ _ t t' x (abs L {m} {m'} cf) t->||t' = abs (x ∷ L) body
  ...
  body y y∉x∷L rewrite
    ...
    subst-fresh x (fv y) t' (x∉FVy y y∉x∷L) =
      lem2-5-1 (m ^' y) (m' ^' y) t t' x (cf y (λ z -> y∉x∷L (there z))) t->||t'


lem2-5-1 _ _ t t' x (beta L cf n->||n') t->||t' = {!   !}
