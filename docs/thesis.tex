...
The reason why the intersection \texttt{ITypeI} is defined as a list of
strict types \texttt{IType} in line 9, is due to the (usually) implicit
requirement that the types in \(\mathcal{T}\) be finite. The decision to
use lists as an implementation of fine sets was taken, because the Agda
standard library includes a definition of lists with definitions of list
membership \(\in\) and other associated lemmas, which proved to be
...
\texttt{IType} and \texttt{ITypeI} are somewhat redundant, in that
\texttt{ITypeI} only has one constructor \texttt{∩} and therefore, any
instance of \texttt{IType} in the definition of \texttt{ITypeS} can
simply be replaced by \texttt{List IType}:
...
which allows one to derive the previously \textbf{?verbally/implicitly?}
...
\texttt{(∩ (Data.List.map (λ τk -> (∩ τi ~> τk)) τi) ~> τ)} is difficult
due to the map function, which appears inside the definition.\\
...
\textbf{Maybe too much detail??} After redefining the intersection
types, the definition of the \((arr)\) rule for the type subset relation
is now:
