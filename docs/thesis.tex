...
\addcontentsline{toc}{subsection}{\(\tau_i \equiv \omega\)}
...
\addcontentsline{toc}{subsection}{\(\tau_i \equiv [\tau_1,\hdots,\tau_n]\)}
...
First, we assume \((1): \Gamma \Vdash m : \tau_i \leadsto \tau_j\),
\((2): [\tau] \subseteq^A_\ell \tau_j\) from \cref{figure:shapemYm}.\\
Since we also have \(\Gamma \Vdash_\ell Y_A m : \tau_i\), where we know
that \(\tau_i\) is not empty, by \cref{Lemma:Ymaxl}, we have some
\(\tau'_i\) s.t. \((3): \Gamma \Vdash m : \tau'_i \leadsto \tau'_i\) and
\((4): \tau_i \subseteq^A_\ell \tau'_i\).\\
