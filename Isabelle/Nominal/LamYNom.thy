...
inductive 
  pbeta :: "trm \<Rightarrow> trm \<Rightarrow> bool" ("_ \<ggreater> _" [80,80] 80)
where
  refl[intro]: "(Var x) \<ggreater> (Var x)"
...
| abs[intro]: "\<lbrakk> M \<ggreater> M' \<rbrakk> \<Longrightarrow> Lam [x]. M \<ggreater> Lam [x]. M'"
| beta[intro]: "\<lbrakk> atom x \<sharp> N ; atom x \<sharp> N' ; M \<ggreater> M' ; N \<ggreater> N' \<rbrakk> \<Longrightarrow> App (Lam [x]. M) N \<ggreater> M'[x ::= N']"
...
lemma fresh_in_pbeta: "s \<ggreater> s' \<Longrightarrow> atom (x::name) \<sharp> s \<Longrightarrow>  atom x \<sharp> s'"
...
lemma pbeta_lam_case_ex: "(Lam [x]. s) \<ggreater> s' \<Longrightarrow> \<exists>t. s' = Lam [x]. t \<and> s \<ggreater> t"
proof (cases "(Lam [x]. s)" s' rule:pbeta.cases, simp)
  case (goal1 _ _ x')
    then have 1: "s \<ggreater> ((x' \<leftrightarrow> x) \<bullet> M')" using pbeta.eqvt by (metis Abs1_eq_iff(3) Nominal2_Base.swap_self add_flip_cancel flip_commute flip_def permute_flip_cancel2 permute_plus)
...
lemma pbeta_cases_2:
  shows "atom x \<sharp> t \<Longrightarrow> App (Lam [x]. s) t \<ggreater> a2 \<Longrightarrow> 
    (\<And>s' t'. a2 = App (Lam [x]. s') t' \<Longrightarrow> atom x \<sharp> t' \<Longrightarrow> s \<ggreater> s' \<Longrightarrow> t \<ggreater> t' \<Longrightarrow> P) \<Longrightarrow>
    (\<And>t' s'. a2 = s' [x ::= t'] \<Longrightarrow> atom x \<sharp> t \<Longrightarrow> atom x \<sharp> t' \<Longrightarrow> s \<ggreater> s' \<Longrightarrow> t \<ggreater> t' \<Longrightarrow> P) \<Longrightarrow> P"
apply atomize_elim
proof (cases "App (Lam [x]. s) t" a2 rule:pbeta.cases, simp)
case goal1 
  then obtain s'' where 1: "M' = Lam [x]. s''" "s \<ggreater> s''" using pbeta_lam_case_ex by blast
...
case (goal2 xx _ ss)
  ...
  from goal2 have 2: "s \<ggreater> ((x \<leftrightarrow> xx) \<bullet> M')"
...
next
case (beta y q q' p p')
  have "App ((Lam [y]. p) [x ::= t]) (q [x ::= t]) \<ggreater> (p' [x ::= t'])[y ::= q'[x ::= t']]"
...
case (beta x Q Qmax P Pmax)
  ...
  proof -
  case (goal2 Q' P')
    with beta have "P' \<ggreater> Pmax" "Q' \<ggreater> Qmax" by simp+
    thus ?case unfolding goal2 apply (rule_tac Lem2_5_1) by simp+
  next
  case (goal1 P' Q')
    with beta have ih: "P' \<ggreater> Pmax" "Q' \<ggreater> Qmax" by simp+
...
lemma pbeta_refl[intro]: "s \<ggreater> s"
