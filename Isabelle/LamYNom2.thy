...
next
case (abs M M' x)
  from abs(3) obtain \<pi> \<tau> where 1: "\<sigma> = \<pi> \<rightarrow> \<tau>" "\<Gamma> \<turnstile> Lam [x]. M : \<pi> \<rightarrow> \<tau>"
    by (metis lam.distinct(3) lam.distinct(7) wt_terms.simps)
...
inductive 
  beta_Y :: "lam \<Rightarrow> lam \<Rightarrow> bool" (" _ \<Rightarrow> _" [80,80] 80)
where
  red_L[intro]: "\<lbrakk> M \<Rightarrow> M' \<rbrakk> \<Longrightarrow> App M N \<Rightarrow> App M' N"
| red_R[intro]: "\<lbrakk> N \<Rightarrow> N' \<rbrakk> \<Longrightarrow> App M N \<Rightarrow> App M N'"
| abs[intro]: "\<lbrakk> M \<Rightarrow> M' \<rbrakk> \<Longrightarrow> Lam [x]. M \<Rightarrow> Lam [x]. M'"
...
lemma beta_Ytyp:
  ...
  shows "\<Gamma> \<turnstile> M' : \<sigma>"
using assms(2,1) 
proof (nominal_induct  M M' arbitrary: \<Gamma> \<sigma> rule: beta_Y.strong_induct)
case (red_L M M' N)
  from red_L(3) obtain \<tau> where 1: "\<Gamma> \<turnstile> M : \<tau> \<rightarrow> \<sigma>" "\<Gamma> \<turnstile> N : \<tau>"
    by (metis lam.distinct(1) lam.distinct(7) lam.distinct(9) lam.eq_iff(2) wt_terms.simps)
  with red_L(2) have "\<Gamma> \<turnstile> M' : \<tau> \<rightarrow> \<sigma>" by simp
...
inductive 
  pbeta :: "lam \<Rightarrow> lam \<Rightarrow> bool" ("_ \<Rightarrow>\<parallel> _" [80,80] 80)
where
...
nominal_inductive pbeta
 avoids beta: "x" | abs: "x" (*don't understand what this does exactly or why we need it...*)
  apply (simp_all add: fresh_star_def fresh_Pair fresh_fact fresh_type)
done
...
| app[intro]: "\<lbrakk> not_abst M ; M >>> M' ; N >>> N' \<rbrakk> \<Longrightarrow> App M N >>> App M' N'"
...
proof (cases "(Lam [x]. s)" s' rule:pbeta.cases, simp)
  case (goal1 _ _ x')
    ...
    from goal1 have "atom x \<sharp> (Lam [x']. M')"  using fresh_in_pbeta by (meson lam.fresh(3) list.set_intros(1))
    with 2 have "s' = Lam [x]. ((x' \<leftrightarrow> x) \<bullet> M')" unfolding goal1 by (metis "2" flip_fresh_fresh goal1(3) lam.fresh(3) list.set_intros(1))
